...
import static ru.practicum.shareit.model.Status.REJECTED;
import static ru.practicum.shareit.model.Status.WAITING;
...
public class BookingServiceImpl implements BookingService {
    ...
    public List<GetBookingDto> getUserBookings(long userId, String stateString, int from, int size) {
        ...
        switch (state) {
            ...
            case FUTURE:
                ...
                break;
            case WAITING:
                bookings = bookingStorage.findAllByBookerAndStatus(user, WAITING, pageable);
                break;
            case REJECTED:
                bookings = bookingStorage.findAllByBookerAndStatus(user, REJECTED, pageable);
                break;
            default:
    ...
    public List<GetBookingDto> getOwnerBookings(long userId, String stateString, int from, int size) {
        ...
        switch (state) {
            case ALL:
                bookings = new PageImpl<>(bookingStorage.findAllByItemOwner(user, pageable));
                break;
            case CURRENT:
            ...
            case FUTURE:
                ...
                break;
            case WAITING:
                bookings = bookingStorage.findAllByItemOwnerAndStatus(user, WAITING, pageable);
                break;
            case REJECTED:
                bookings = bookingStorage.findAllByItemOwnerAndStatus(user, REJECTED, pageable);
                break;
            default:
                throw new MethodArgumentException(String.format("Illegal state = %s", state));
        }
