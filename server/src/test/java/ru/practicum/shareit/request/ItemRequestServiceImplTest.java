...
class ItemRequestServiceImplTest {
    ...
    private static Page<ItemRequest> listOfRequests;
    ...
    static void beforeAll() {
        ...
        List<ItemRequest> listOfR = new ArrayList<>();
        for (int i = 1; i < 21; i++) {
            listOfR.add(request.toBuilder().id(i + 1L).build());
        }
        listOfRequests = new PageImpl<>(listOfR);
    }
    ...
    void getAllRequestsByUserId() {
        ...
        when(requestStorage.getAllByRequester(any(User.class), any(Sort.class)))
                .thenReturn(listOfRequests.getContent());
